...
```cmake
include(${TINYPLUG_DIR}/tools/pagetables/make_pagetable.cmake)
...
./build-release/examples/my_plugin/MyPlugin_pagetable_manifest > /tmp/MyPlugin_manifest.json
```
...
```bash
python3 tools/pagetables/generate_pages.py /tmp/MyPlugin_manifest.json \
    examples/my_plugin/assets/MyPluginPages.xml
```
