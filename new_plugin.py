...
def main():
    ...
    parser = ArgumentParser(
        prog="new_plugin.py",
        description="Create a new plug-in from template/ (a simple gain effect)."
    )
    ...
    if args.dest:
        ...
        dest_dir = os.path.join(here, "examples", dir_name)
    ...
    # If no destination, add to the example plugins build.
    if args.dest == None:
        print("-- Adding to build.")
        build_file = os.path.join(here, "examples", "CMakeLists.txt")
        # append 'add_subdirectory'
        with open(build_file, "a") as f:
            f.write(f"add_subdirectory({dir_name})\n")

        # sort lines alphabetically (the leading comment keeps its place at the top)
